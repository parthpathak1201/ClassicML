...
RidgeRegression::RidgeRegression(GradientDescent<LinearParams, Matrix> *ext_opt)
    : bias(0.0), owned_optimizer(nullptr), optimizer(ext_opt) {
    if (!optimizer) {
        throw std::invalid_argument("External optimizer must not be null");
    }
}

void RidgeRegression::docs() {
...
void RidgeRegression::compute_gradients(const Matrix &X, const Vec &y, const size_t *start, const size_t *end, const LinearParams &params, LinearParams &gradients) {
    ...
    for (size_t i = 0; i < batch; ++i) {
        const size_t idx = start[i];
        const double pred = std::inner_product(X[idx].begin(), X[idx].end(), params.weights.begin(), 0.0) +
                            params.bias;
        const double err = pred - y[idx];

        gradients.bias += err;
        for (size_t j = 0; j < n_features; ++j) {
            gradients.weights[j] += err * X[idx][j];
        }
    }

    for (size_t j = 0; j < n_features; ++j) {
        gradients.weights[j] += optimizer->get_l2_lambda() * params.weights[j];
    }
}
...
Vec RidgeRegression::predict(const Matrix &X) {
    validate_predict_matrix(X, weights.size());

    Vec out;
    out.reserve(X.size());
    for (const auto &sample : X) {
        out.push_back(std::inner_product(sample.begin(), sample.end(), weights.begin(), 0.0) + bias);
    }
    return out;
}

double RidgeRegression::score(const Vec &y_true, const Vec &y_pred) {
    return regression_r2(y_true, y_pred);
}
...
LassoRegression::LassoRegression(GradientDescent<LinearParams, Matrix> *ext_opt)
    : bias(0.0), owned_optimizer(nullptr), optimizer(ext_opt) {
    if (!optimizer) {
        throw std::invalid_argument("External optimizer must not be null");
    }
...
void LassoRegression::compute_gradients(const Matrix &X, const Vec &y, const size_t *start, const size_t *end, const LinearParams &params, LinearParams &gradients) {
    ...
    for (size_t j = 0; j < n_features; ++j) {
        const double sign = (params.weights[j] > 0.0) ? 1.0 : (params.weights[j] < 0.0 ? -1.0 : 0.0);
        gradients.weights[j] += l1 * sign;
    }
}
...
void LassoRegression::fit(const Matrix &X, const Vec &y, size_t n_epochs, bool verbose) {
    ...
    for (size_t epoch = 0; epoch < n_epochs; ++epoch) {
        ...
        auto grad_func = [this, &X, &y](const Matrix &, const size_t *s, const size_t *e,
                                        const LinearParams &p, LinearParams &g) {
            compute_gradients(X, y, s, e, p, g);
        };
        optimizer->step(params, X, grad_func, epoch);
        ...
        const double l1 = optimizer->get_l2_lambda();
        costs.push_back(total_error / static_cast<double>(n_samples) +
                        (l1 / static_cast<double>(n_samples)) * l1_cost);
...
Vec LassoRegression::predict(const Matrix &X) {
    validate_predict_matrix(X, weights.size());
...
double LassoRegression::score(const Vec &y_true, const Vec &y_pred) {
    return regression_r2(y_true, y_pred);
}
...
void KMeans::docs() {
    ...
    Log::info("Constructor: KMeans(k, max_iter, n_restarts, init_type, seed)");
    Log::info("fit(X, verbose), predict(X), get_centroids(), get_inertia()");
    Log::divider();
}

void KMeans::init_cluster_size() {
    clusters.assign(static_cast<size_t>(k), Matrix{});
    for (auto &cluster : clusters) {
        cluster.reserve(X_train.size());
    }
}

Matrix KMeans::init_centroids(const Matrix &X, int restart_index) {
    std::mt19937 rng(seed_ + static_cast<unsigned>(restart_index));
    ...
    for (int i = 1; i < k; ++i) {
        ...
        for (size_t j = 0; j < X.size(); ++j) {
            ...
            for (const auto &centroid : cents) {
                min_d = std::min(min_d, static_cast<double>(square_distance(X[j], centroid)));
            }
            distances[j] = min_d;
        }

        std::discrete_distribution<int> weighted(distances.begin(), distances.end());
        cents.push_back(X[static_cast<size_t>(weighted(rng))]);
    }
...
void KMeans::assign_clusters() {
    ...
    for (const auto &sample : X_train) {
        ...
        for (int i = 0; i < k; ++i) {
            const double distance = static_cast<double>(square_distance(sample, centroids[static_cast<size_t>(i)]));
        ...
        clusters[static_cast<size_t>(best_index)].push_back(sample);
    }
}

void KMeans::update_centroids(int restart_index) {
    std::mt19937 rng(seed_ + static_cast<unsigned>(restart_index));
    std::uniform_int_distribution<int> pick(0, static_cast<int>(X_train.size()) - 1);

    for (int i = 0; i < k; ++i) {
        auto &cluster = clusters[static_cast<size_t>(i)];
        if (cluster.empty()) {
            centroids[static_cast<size_t>(i)] = X_train[static_cast<size_t>(pick(rng))];
            continue;
        }

        Vec next(X_train[0].size(), 0.0);
        ...
        centroids[static_cast<size_t>(i)] = next;
    }
}

double KMeans::run_once(int restart_index) {
    centroids = init_centroids(X_train, restart_index);
    init_cluster_size();

    for (int iter = 0; iter < max_iter; ++iter) {
        Matrix old_centroids = centroids;
        assign_clusters();
        update_centroids(restart_index);

        bool converged = true;
        for (int i = 0; i < k; ++i) {
            if (square_distance(centroids[static_cast<size_t>(i)], old_centroids[static_cast<size_t>(i)]) > 1e-6) {
                converged = false;
                break;
            }
        }
...
void KMeans::fit(const Matrix &X, bool verbose) {
    if (X.empty() || X[0].empty()) {
        throw std::invalid_argument("fit: empty data");
    }
    ...
    for (int r = 0; r < n_restarts; ++r) {
        const double inertia = run_once(r);
        if (verbose) {
...
double KMeans::score(const Matrix &X) {
    double total = 0.0;
    for (const auto &point : X) {
        double min_distance = -1.0;
        for (const auto &centroid : centroids) {
            const double distance = static_cast<double>(square_distance(point, centroid));
...
std::vector<int> KMeans::predict(const Matrix &X) {
    ...
    for (const auto &point : X) {
        ...
        for (int i = 0; i < k; ++i) {
            const double distance = static_cast<double>(square_distance(point, centroids[static_cast<size_t>(i)]));
            if (min_distance < 0.0 || distance < min_distance) {
                min_distance = distance;
                best_index = i;
            }
...
void GaussianNB::docs() {
    Log::header("GaussianNB");
    Log::info("Gaussian Naive Bayes classifier (per-class means/variances).");
    Log::info("fit(X, y, verbose), predict(X), predict_prob(X), score(y_true, y_pred) -> accuracy");
    Log::divider();
}

double GaussianNB::log_gaussian(double x, double mean, double variance) {
    const double var = std::max(variance, 1e-9);
    static const double pi = 3.14159265358979323846;
    const double diff = x - mean;
    return -0.5 * (std::log(2.0 * pi * var) + (diff * diff) / var);
}

Matrix GaussianNB::give_means(const Matrix &X, const Vec &y, const std::vector<int> &cls) {
    Matrix all_means;

    for (int c : cls) {
        Vec class_mean(X[0].size(), 0.0);
        int count = 0;
        ...
        for (double &value : class_mean) {
            value /= static_cast<double>(count > 0 ? count : 1);
        }
        all_means.push_back(class_mean);
    }

    return all_means;
}

Matrix GaussianNB::give_variances(const Matrix &X, const Vec &y, const std::vector<int> &cls,
                                  const Matrix &all_means) {
    Matrix all_variances;

    for (size_t k = 0; k < cls.size(); ++k) {
        Vec class_variance(X[0].size(), 0.0);
        int count = 0;

        for (size_t i = 0; i < X.size(); ++i) {
            if (static_cast<int>(y[i]) != cls[k]) {
                continue;
            }

            for (size_t j = 0; j < X[0].size(); ++j) {
                const double diff = X[i][j] - all_means[k][j];
                class_variance[j] += diff * diff;
            }
            count++;
        }

        for (double &value : class_variance) {
            value = value / static_cast<double>(count > 0 ? count : 1) + 1e-9;
        }
        all_variances.push_back(class_variance);
    }

    return all_variances;
}

Vec GaussianNB::give_priors(const Vec &y) {
    Vec result(classes.size(), 0.0);

    for (size_t k = 0; k < classes.size(); ++k) {
        int count = 0;
        for (double label : y) {
            if (static_cast<int>(label) == classes[k]) {
                count++;
            }
        }
        result[k] = static_cast<double>(count) / static_cast<double>(y.size());
    }

    return result;
}

void GaussianNB::init() {
    means = give_means(X_train, y_train, classes);
    variances = give_variances(X_train, y_train, classes, means);
    priors = give_priors(y_train);
}

void GaussianNB::extract_classes() {
    std::set<int> unique;
    for (double label : y_train) {
        unique.insert(static_cast<int>(label));
    }
    classes.assign(unique.begin(), unique.end());
}

void GaussianNB::fit(const Matrix &X, const Vec &y, bool verbose) {
    validate_xy(X, y, "fit");
    X_train = X;
    y_train = y;
    extract_classes();
...
Vec GaussianNB::log_posteriors(const Vec &sample) {
    Vec scores;
    ...
    for (size_t k = 0; k < classes.size(); ++k) {
        ...
        for (size_t j = 0; j < sample.size(); ++j) {
            score += log_gaussian(sample[j], means[k][j], variances[k][j]);
        }
...
Vec GaussianNB::predict(const Matrix &X) {
    Vec result;
    ...
    for (const auto &sample : X) {
        ...
        result.push_back(classes[static_cast<size_t>(std::distance(scores.begin(), best))]);
    }

    return result;
}

Matrix GaussianNB::predict_prob(const Matrix &X) {
    Matrix result;
    result.reserve(X.size());

    for (const auto &sample : X) {
        result.push_back(log_posteriors(sample));
    }
...
void SVM::fit(const Matrix &X, const Vec &y, bool verbose) {
    // The model is preprocessing-agnostic. X is exactly what the caller wants SVM to train on
    // (the demos pass already-standardized data), and predict(X) below uses that same feature space.
    validate_xy(X, y, "fit");
    const size_t n_samples = X.size();
    const size_t n_features = X[0].size();
    ...
    for (int epoch = 0; epoch < n_epochs; ++epoch) {
        for (size_t i = 0; i < n_samples; ++i) {
            ...
            if (condition >= 1.0) {
                ...
                for (size_t j = 0; j < n_features; ++j) {
                    weights[j] -= lr * (lambda * weights[j] - y_svm[i] * X[i][j]);
                }
                bias += lr * y_svm[i];
            }
        }
        ...
        if (log_epoch_milestone(static_cast<size_t>(epoch), static_cast<size_t>(n_epochs), verbose)) {
            Vec preds = predict(X);
            Log::epoch(epoch + 1, n_epochs, Color::Yellow, " acc=", Color::White, score(y, preds));
        }
    }

    if (verbose) {
        Log::success("SVM fit complete. train accuracy=", score(y, predict(X)));
    }
}
