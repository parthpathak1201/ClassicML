...
void shuffle_dataset(Matrix &X, Vec &y, int seed) {
    ...
    for (size_t idx : indices) {
        ...
        if (!y.empty()) {
            shuffled_y.push_back(y[idx]);
        }
    }
    X = std::move(shuffled_X);
    if (!y.empty()) {
        y = std::move(shuffled_y);
    }
}
    ...
    while (std::getline(file, line)) {
        ...
        if (last_col_is_label) {
            ...
            for (size_t j = 0; j + 1 < fields.size(); ++j) {
                row.push_back(std::stod(fields[j]));
            }
            X.push_back(row);
...
std::pair<Matrix, Vec> generate_blobs(int n, int k, int seed, double cluster_std) {
    std::mt19937 rng(seed);
    ...
    shuffle_dataset(X, y, seed);
    return {X, y};
}

std::pair<Matrix, Vec> generate_linearly_separable(int n, int seed) {
    std::mt19937 rng(seed);
...
std::pair<Matrix, Vec> generate_xor(int n, int seed, double noise) {
    std::mt19937 rng(seed);
    ...
    auto add_quadrant = [&](double sx, double sy, double label) {
        for (int i = 0; i < per_quadrant; ++i) {
            double x = sx * coord_dist(rng) + noise_dist(rng);
            double y_val = sy * coord_dist(rng) + noise_dist(rng);
            X.push_back({x, y_val});
            y.push_back(label);
        }
    };
...
std::pair<Matrix, Vec> generate_moons(int n, int seed, double noise) {
    std::mt19937 rng(seed);
    std::normal_distribution<double> noise_dist(0.0, noise);
    ...
    for (int i = 0; i < half; ++i) {
        ...
        double x = std::cos(angle) + noise_dist(rng);
        double y_val = std::sin(angle) + noise_dist(rng);
        X.push_back({x, y_val});
        y.push_back(0.0);
    }
    ...
    for (int i = 0; i < half; ++i) {
        double angle = M_PI * static_cast<double>(i) / half;
        double x = std::cos(angle) + 0.5 + noise_dist(rng);
        double y_val = std::sin(angle) - 0.2 + noise_dist(rng);
        X.push_back({x, y_val});
        y.push_back(1.0);
    }
...
void print_matrix(const Matrix &X, int n_rows) {
...
void print_vec(const Vec &v, int n_elems) {
